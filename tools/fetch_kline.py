#!/usr/bin/env python3
"""
从 stocklist.csv 读取股票池并用 Tushare 抓取日线K线（固定 qfq，全量覆盖）
...
# --------------------------- 全局日志配置 --------------------------- #
LOG_FILE = Path(__file__).resolve().parent / "fetch.log"
logging.basicConfig(
    level=logging.INFO,
...
def main():
    ...
    default_start = (dt.date.today() - dt.timedelta(days=8*30+1)).strftime("%Y%m%d")
    parser.add_argument("--start", default=default_start, help="起始日期 YYYYMMDD 或 'today'")
    parser.add_argument("--end", default="today", help="结束日期 YYYYMMDD 或 'today'")
    parser.add_argument(
        "--stocklist",
        type=Path,
        default=Path(__file__).resolve().parent.parent / "stocklist.csv",
