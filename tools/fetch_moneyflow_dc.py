...
# --------------------------- 全局日志配置 --------------------------- #
LOG_FILE = Path(__file__).resolve().parent / "fetch_moneyflow_dc.log"
logger = logging.getLogger("fetch_moneyflow_dc")
...
def main():
    ...
    parser.add_argument(
        "--out",
        default=Path(__file__).resolve().parent.parent / "public" / "data" / "moneyflow_dc",
        help="输出目录",
    )
    parser.add_argument(
        "--check-missing",
    ...
    if args.date:
        ...
        dates = get_all_dates_between(month_ago.strftime("%Y%m%d"), today.strftime("%Y%m%d"))

    # 按周分批拉取，避免单月数据量过大导致API截断
    week_ranges = get_week_ranges(args.start, args.end)
    print(f"准备拉取数据 ({args.start} ~ {args.end})，共 {len(week_ranges)} 周")
